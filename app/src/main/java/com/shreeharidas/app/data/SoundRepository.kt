...
class SoundRepository(private val context: Context) {
    ...
    fun getDefaultSounds(): List<SoundOption> {
        return listOf(
            SoundOption(
                name = "Kunj Vihari - Shree Haridas",
                uri = buildResourceUri(R.raw.kunjvihari_shreeharidas),
                type = SoundType.DEFAULT,
                isDefault = true
            ),
            SoundOption(
                name = "Shree Haridas",
                uri = buildResourceUri(R.raw.shreeharidas),
                type = SoundType.DEFAULT,
                isDefault = true
            ),
            SoundOption(
                name = "Shyama Pyari - Kunj Vihari",
                uri = buildResourceUri(R.raw.shyamapyari_kunjvihari),
                type = SoundType.DEFAULT,
                isDefault = true
            )
        )
    ...
    private fun buildResourceUri(resId: Int): String {
        return "android.resource://${context.packageName}/$resId"
    }

    /** Returns the URI string for the default sound (fallback). */
    fun getDefaultSoundUri(): String {
        return buildResourceUri(R.raw.kunjvihari_shreeharidas)
    }
    ...
    fun resolveRawResourceId(uriString: String): Int? {
        ...
        if (uriString.startsWith("default:")) {
            return when (uriString.removePrefix("default:")) {
                "kunjvihari_shreeharidas" -> R.raw.kunjvihari_shreeharidas
                "shreeharidas" -> R.raw.shreeharidas
                "shyamapyari_kunjvihari" -> R.raw.shyamapyari_kunjvihari
                else -> R.raw.kunjvihari_shreeharidas
            }
        }
        ...
        if (uriString.startsWith("android.resource://")) {
            try {
                ...
                if (resId != null) {
                    val knownIds = listOf(
                        R.raw.kunjvihari_shreeharidas,
                        R.raw.shreeharidas,
                        R.raw.shyamapyari_kunjvihari
                    )
    ...
    fun resolveUri(uriString: String): Uri? {
        return try {
            if (uriString.startsWith("default:")) {
                ...
                Uri.parse(buildResourceUri(resId))
            } else {
                Uri.parse(uriString)
            }
        } catch (_: Exception) {
    ...
    fun getSoundDisplayName(uriString: String, soundType: SoundType): String {
        return when {
            uriString.startsWith("default:") -> {
                when (uriString.removePrefix("default:")) {
                    "kunjvihari_shreeharidas" -> "Kunj Vihari - Shree Haridas"
                    "shreeharidas" -> "Shree Haridas"
                    "shyamapyari_kunjvihari" -> "Shyama Pyari - Kunj Vihari"
                    else -> "Default Sound"
                }
            }
